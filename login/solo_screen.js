...
const clockIcon = document.querySelector('.tool-icon:nth-child(1)'); // Icon đồng hồ
...
function hideAllPopups() {
    //if (typeof participantsPopup !== 'undefined') {
...
document.addEventListener('click', (event) => {
    if (!event.target.closest('.tool-icon')  && !event.target.closest('.timer-section')) {
        hideAllPopups();
    }
});
